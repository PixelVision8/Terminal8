...
function Take(options, state)
  ...
  state.DisplayMessage(textTemplates.actionErrors.cantTake)
...
function Examine(options, state)
  ...
  state.DisplayMessage(textTemplates.actionErrors.cantExamine)
  ...
  for i = 1, #state.inventory do
    if(state.inventory[i].name == itemName) then
      ...
      if(targetItem.needs ~= nil and ItemInInventory(targetItem.needs, state) == nil) then
        ...
        -- TODO need to pass the required item in 
        state.DisplayMessage(textTemplates.actionErrors.need)--"You need ".. targetItem.needs .. " to use this.")
        ...
        if(targetItem.use ~= nil) then
          ...
          state.DisplayMessage(textTemplates.actionErrors.cantUse)
        end
